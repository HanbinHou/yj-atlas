---
author: Edward Allen, Patrick Rand, Jason Miller
category: 材料与构造
coverImage: /images/books/architectural-detailing-function-constructability-aesthetics/01.png
hiddenContent: '<h3>📥 下载链接</h3>

  <p><a href=" https://pan.baidu.com/s/1Eq4WEyeEi3HJRFqx3acA0g?pwd=b3jt" target="_blank"
  rel="noopener">百度网盘：建筑氛围 PDF</a></p>

  <p>提取码：<code>yjatlas</code></p>

  <p style="margin-top: 1rem; font-size: 0.85rem; color: var(--color-text-muted);">如链接失效，请在公众号「咏迹探所」留言书名。</p>'
price: 4
readingPath: beginner
summary: 本书是建筑细部设计的行业标准指南，系统阐释将功能性、可建造性与美学融为一体以成就高性能建筑的原则。通过模式化的最佳实践，讲解为何细部设计至关重要，以及如何设计能控水隔热、适应各种运动且易于施工的细部。第四版依据最新国际建筑规范与节能标准全面更新，融入预制装配、模块化建造及可持续韧性设计的最新知识，以大量清晰插图示范不漏、不凝、不裂的细部构造方法。
tags:
- 建筑细部
- 构造原理
- 施工性
- 建筑性能
- 可持续设计
title: 'Architectural Detailing: Function, Constructability, Aesthetics'
unlockCode: yj2024
year: 2024
---

本书最初由著名建筑教育家Edward Allen撰写，自问世以来便成为连接建筑设计构思与建造现实的核心教材。第四版邀请Patrick Rand与Jason Miller加入修订，在延续经典理论框架的同时，大量吸纳当代建造技术、法规更新与可持续实践，使内容始终站在构造教育的前沿。

全书以“功能-可建造性-美学”三元模型为骨架，逐层拆解细部的设计逻辑。章节从控制水、热、水蒸气、声音及运动等物理因素入手，逐步过渡到材料连接、施工顺序与公差配合，最后结合预制化、模块化的新趋势，展示细部如何在高性能建筑与韧性设计中发挥关键作用。每一章都配备模式图例与实际案例对比，帮助读者建立从原则到图纸的转化能力。

本书核心观点在于，细部不是装饰的附属，而是建筑物理性能的载体。作者提出一系列可以覆盖大多数情境的“细部模式”，将看似复杂的构造问题归纳为可传承的解决方案，强调好的细部必须同时满足“不漏、不坏、好看、好做”四重标准。这种将性能与建造逻辑前置的设计教学法，深刻影响了北美及全球的建筑技术教育。

自出版以来，该书被广泛用作建筑学专业构造课程的主要参考书，也是职业建筑师准备注册考试和深化施工图的重要工具。其跨版次的持续更新，见证了建筑构造从传统湿作业向装配式、数字化建造的演进，尤其在第四版对IBC和IECC规范的紧跟、以及对隐含碳与气候适应性的关注，进一步巩固了其作为当代细部设计权威指南的地位。